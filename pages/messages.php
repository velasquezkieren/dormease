...
if (isset($_GET['u_ID'])) {
    $recipient_ID = $_GET['u_ID']; // Get the u_ID from the URL

    // Fetch the selected user's details directly from the database if not found in conversations
    $userQuery = "
        SELECT u.u_ID, 
               CONCAT(u.u_FName, ' ', COALESCE(u.u_MName, ''), ' ', u.u_LName) AS full_name, 
               u.u_Account_Type,
               u.u_Picture AS userPicture
        FROM user u
        WHERE u.u_ID = '$recipient_ID' AND u.u_ID <> '$u_ID'
    ";
    $userResult = $con->query($userQuery);
    if ($userResult->num_rows > 0) {
        $currentConversation = $userResult->fetch_assoc();
    }
